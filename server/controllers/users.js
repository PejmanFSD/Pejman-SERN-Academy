...
module.exports.index = async (req, res) => {
  ...
  const users = await User.getAllUsers(); // The "getAllUsers" function has been already created in the model
...
module.exports.changePassword = async (req, res) => {
    ...
    if (currentPassword === newPassword) { // If not
    ...
    if (!isStrongPassword(newPassword)) { // The "isStrongPassword" function has been already created in the utils folder
        // Sending the error message to the front-end by the status code of 400
        return res.status(400).json({
...
module.exports.deleteUser = async (req, res) => {
    ...
    if (!deletedUser) { // If there's no user with the fetched id
